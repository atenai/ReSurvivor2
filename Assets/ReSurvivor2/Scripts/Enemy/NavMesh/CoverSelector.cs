...
public class CoverSelector : MonoBehaviour
{
	...
	bool FindBestCover()
	{
		...
		foreach (var cp in coverPoints)
		{
			...
			Vector3 dir = cp.Position - InGameManager.SingletonInstance.PlayerManager.transform.position;
		...
		foreach (var cp in coverPoints)
		{
			...
			float dP = Vector3.Distance(InGameManager.SingletonInstance.PlayerManager.transform.position, cp.Position);
			if (dP > bestScore)
			{
				bestScore = dP;
				best = cp;
