...
public class CoverSelector : MonoBehaviour
{
	...
	bool FindBestCover()
	{
		...
		foreach (var cp in coverPoints)
		{
			...
			Vector3 dir = cp.Position - PlayerManagerPresenter.SingletonInstance.transform.position;
		...
		foreach (var cp in coverPoints)
		{
			...
			float dP = Vector3.Distance(PlayerManagerPresenter.SingletonInstance.transform.position, cp.Position);
			if (dP > bestScore)
			{
				bestScore = dP;
				best = cp;
