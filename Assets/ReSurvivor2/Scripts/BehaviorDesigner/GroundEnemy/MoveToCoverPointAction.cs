...
public class MoveToCoverPointAction : Action
{
	...
	Vector3 FindBestCover()
	{
		...
		foreach (var coverPoint in groundEnemy.GetCoverPoints)
		{
			...
			Vector3 dir = coverPoint.Position - InGameManager.SingletonInstance.PlayerManager.transform.position;
			...
			RaycastHit hit;
			if (Physics.Raycast(InGameManager.SingletonInstance.PlayerManager.transform.position, dir.normalized, out hit, dist, obstructionMask))
			{
		...
		foreach (var coverPoint in groundEnemy.GetCoverPoints)
		{
			...
			float distanceScore = Vector3.Distance(InGameManager.SingletonInstance.PlayerManager.transform.position, coverPoint.Position);
