...
public class EnemyMovePatrolPointRayCastAction : Action
{
    ...
    {
        //巡回地点が設定されていなければ
        if (enemyRayCast.PatrolPoints.Length == 0)
        {
            //切り上げる
            return;
        }
        //現在選択されている配列の座標を巡回地点の座標に代入
        patrolPointPos = enemyRayCast.PatrolPoints[pointNumber].transform.position;
        //配列の中から次の巡回地点を選択（必要に応じて繰り返し）
        pointNumber = (pointNumber + 1) % enemyRayCast.PatrolPoints.Length;
    }
